...
module EbDeployer
  ...
  #
  # Swap an elastic beanstalk environment
  #
  # @param [Hash] opts
  #
  ...
  #
  # Destroys inactive elastic beanstalk environment
  #
  # @param [Hash] opts
  #
  # @option opts [Symbol] :application *required* Application name, this
  ...
  # @option opts [Symbol] :template_name. Specifies the environement template you wish
  #   to use to build your environment.
  def self.destroy_inactive(opts)
    if region = opts[:region]
      Aws.config.update(:region => region)
    end
  ...
  def self.cli_parser(options)
    OptionParser.new do |opts|
      ...
      opts.on("--destroy-inactive", "Destroy inactive Elasticbeanstalk environments under the application which have specified environment as name prefix") do |v|
        options[:action] = :destroy_inactive
      end
      ...
      opts.on("--swap", "Swap active environment witn inactive environment") do |v|
        options[:action] = :swap
      end
